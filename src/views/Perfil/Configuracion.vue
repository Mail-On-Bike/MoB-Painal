<template>
						...
						<div class="row">
								...
								<div class="profile-header" style="">
									...
									<div class="profile-info" style="">
										...
										<div class="indicators">
											<i class="fas fa-bicycle"></i> {{user.clienteAsignado.biciEnvios}} &nbsp;
											<i class="fas fa-road"></i> {{user.clienteAsignado.kilometros}}km. &nbsp;
											<i class="fas fa-leaf"></i> {{user.clienteAsignado.CO2Ahorrado}} &nbsp;
											<i class="fas fa-deaf"></i> {{user.clienteAsignado.ruido}}h.
										</div>
									</div>
								</div>
							</div>
							<div class="col-12 row profile-content" style="margin-top:15px;">
								<div class="col-12 profile-title">
									...
									<div>
										<button class="btn-sm " @click="detallesUsuario"><i class="fas fa-edit"></i></button>
									</div>
								...
								<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12" style="min-height: 62px;">
									<div class="profile-item">
										...
										<div style="display:flex;justify-content:space-between; align-items:center; ">
											<input :type="showPswd ? 'text' : 'password'" value="contraseña" :class="showPswd ? 'pswd-show-input': 'pswd-hide-input'" readonly disabled>
											<button class="btn-sm" @click="changePassword()"><i class="fas fa-edit"></i></button>
	...
	<modal-detalles-usuario @closeModal="closeModal" @updateProfile="updateProfile" :showModal="showModal" :userProfile="userProfile"></modal-detalles-usuario>
	<modal-change-password @closeModalPswd="closeModalPswd" @updatePassword="updatePassword" :showModalPswd="showModalPswd" ></modal-change-password>
</template>
...
export default {
  ...
  setup() {
		...
		const updateProfile = async (profileUpdated) => {
			...
			const response = await AuthService.updateProfileUser(user.value.id, profileUpdated.value)
		...
		const updatePassword = async (pswd) => {
			...
			if(pswd.value.newPassword === pswd.value.confirmPassword){
				const response = await AuthService.changePassword(user.value.id, pswd.value);
				if(response){
					Swal.fire({
...
.profile-title h3, .profile-title h4, .profile-title h5{
	margin: 0;
}
