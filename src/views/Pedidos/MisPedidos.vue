<template>
	...
	<div class="row">
		...
		<div class="col-md-8 col-sm-12" id="navContainer">
			<div class="pedidosDetails" style="">
				<div>
					<a>Rango: </a> &nbsp;
					<span v-if="currentPage>0" @click="getPedidos(currentPage-1)"><i class="fas fa-chevron-circle-left"></i></span>
					&nbsp; <span>{{mostrandoDesde}} - {{mostrandoHasta}}</span> &nbsp;
					<span v-if="(currentPage +1 ) < totalPages" @click="getPedidos(currentPage+1)"><i class="fas fa-chevron-circle-right"></i></span>
				</div>
				<div>Total: <b>{{totalPedidos}}</b>
				</div>
			</div>
			...
			<div class="tab-content" id="myTabContent">
				<div class="tab-pane fade show active" id="todos" role="tabpanel" aria-labelledby="todos-tab" >
					...
					<div @click="detallesPedido(pedido)" class="pedido-card" v-for="(pedido, index) in pedidosTodosPaginados[currentPageTodos]" :key="index">
						...
						<div style="display:flex; justify-content:space-between; width:100%; padding: 10px;">
							<div style="    display: flex; flex-direction: column;">
								...
								<span class="w-100">{{pedido.direccionRemitente}}, {{pedido.distritoRemitente}}</span>
					...
					<nav aria-label="...">
						<ul class="pagination">
							<li  v-if="currentPageTodos != 1" class="page-item">
								<a class="page-link" href="#" @click="(changePage('todos', parseInt(currentPageTodos) - 1))">
							...
							>
								<a class="page-link" href="#" @click="(changePage('todos', key))">{{key}}</a>
							</li>
							<li v-if="currentPageTodos < Object.keys(pedidosTodosPaginados).length"  class="page-item">
								<a class="page-link" href="#" @click="(changePage('todos', parseInt(currentPageTodos) + 1))">
				...
				<div class="tab-pane fade" id="activos" role="tabpanel" aria-labelledby="activos-tab" >
					...
					<div class="pedido-card" v-for="(pedido, index) in pedidosActivosPaginados[currentPageActivos]" :key="index">
						<div style="display:flex; align-items:center;padding: 10px; ">
							<i class="fas fa-route fa-3x"
								:class="{fc_programado:(pedido.statusId == 1), 
										fc_por_recoger: (pedido.statusId == 2), 
					...
					<nav aria-label="...">
						<ul class="pagination">
							<li  v-if="currentPageActivos != 1" class="page-item">
								<a class="page-link" href="#" @click="(changePage('activos', parseInt(currentPageActivos) - 1))">
									<i class="fas fa-chevron-left"></i>
								</a>
							</li>
							<li 
							...
							>
								<a class="page-link" href="#" @click="(changePage('activos', key))">{{key}}</a>
							</li>
							<li v-if="currentPageActivos < Object.keys(pedidosActivosPaginados).length"  class="page-item">
								<a class="page-link" href="#" @click="(changePage('activos', parseInt(currentPageActivos) + 1))">
									<i class="fas fa-chevron-right"></i>
								</a>
							</li>
				...
				<div class="tab-pane fade" id="finalizados" role="tabpanel" aria-labelledby="finalizados-tab" >
					...
					<div class="pedido-card" v-for="(pedido, index) in pedidosFinalizadosPaginados[currentPageFinalizados]" :key="index">
						<div style="display:flex; align-items:center;padding: 10px; ">
							<i class="fas fa-route fa-3x"
								:class="{fc_entregado:(pedido.statusId == 4), 
										fc_falso_flete: (pedido.statusId == 5)}">
							</i>
						</div>
						<div style="display:flex; justify-content:space-between; width:100%; padding: 10px;">
					...
					<nav aria-label="...">
						<ul class="pagination">
							<li  v-if="currentPageFinalizados != 1" class="page-item">
								<a class="page-link" href="#" @click="(changePage('finalizados', parseInt(currentPageFinalizados) - 1))">
									<i class="fas fa-chevron-left"></i>
								</a>
							</li>
							<li 
							...
							>
								<a class="page-link" href="#" @click="(changePage('finalizados', key))">{{key}}</a>
							</li>
							<li v-if="currentPageFinalizados < Object.keys(pedidosFinalizadosPaginados).length"  class="page-item">
								<a class="page-link" href="#" @click="(changePage('finalizados', parseInt(currentPageFinalizados) + 1))">
									<i class="fas fa-chevron-right"></i>
								</a>
							</li>
				...
				<div class="tab-pane fade" id="anulados" role="tabpanel" aria-labelledby="anulados-tab" >
					...
					<div class="pedido-card" v-for="(pedido, index) in pedidosAnuladosPaginados[currentPageAnulados]" :key="index">
						<div style="display:flex; align-items:center;padding: 10px; ">
							<i class="fas fa-route fa-3x" :class="{fc_anulado:(pedido.statusId == 6)}"></i>
						</div>
						<div style="display:flex; justify-content:space-between; width:100%; padding: 10px;">
							<div style="display: flex; flex-direction: column;">
								<b class="w-100">Origen</b>
					...
					<nav aria-label="...">
						<ul class="pagination">
							<li  v-if="currentPageAnulados != 1" class="page-item">
								<a class="page-link" href="#" @click="(changePage('anulados', parseInt(currentPageAnulados) - 1))">
									<i class="fas fa-chevron-left"></i>
								</a>
							</li>
							<li 
							...
							>
								<a class="page-link" href="#" @click="(changePage('anulados', key))">{{key}}</a>
							</li>
							<li v-if="currentPageAnulados < Object.keys(pedidosAnuladosPaginados).length"  class="page-item">
								<a class="page-link" href="#" @click="(changePage('anulados', parseInt(currentPageAnulados) + 1))">
									<i class="fas fa-chevron-right"></i>
								</a>
							</li>
	...
	<modal-detalles-pedido @closeModal="closeModal" :showModal="showModal" :pedido="pedidoSeleccionado"></modal-detalles-pedido>
</template>
	...
	setup(){
		...
		const getPedidos = async (page = 0, size = 100)=>{
			...
			if(response.data.totalPedidos === 0){
				...
				mostrandoHasta.value = mostrandoDesde.value - 1 + response.data.pedidos.length;
			}
			filterPedidos()
		}
